...
const data = [
    ['Text','Link','Month (1 to 12)', 'Date', 'Hour (From 0 to 23)', 'Minutes','Image URL','Video URL','Video Type','No. of Repetitions (From 1-10 OR \'FOREVER\')','Time Gap between Repetitions (Hours: From 1-24 OR \'WEEKLY\' OR \'MONTHLY\' OR \'YEARLY\')',	'Google Business Profile Type',	'Google Business Profile URL',	'Pinterest Title',	'Pinterest Link',	'Instagram First Comment',	'Facebook First Comment',	'LinkedIn First Comment'	
  ]
  ];
...
while (currentDate <= endDate) {
  for (let j = 0; j < 24; j += 1) { // Hours
    let caption = generateCaptionWithHashtags(captionArray, newHastagArray);
...
function generateCaptionWithHashtags(captionArray, hashtagArray) {
  ...
  const randomHashtags = shuffledHashtags.slice(0, 30);
